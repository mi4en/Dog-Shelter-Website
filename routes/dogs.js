...
router.get("/", function(req, res){
    ...
    if(req.query.search) {
        ...
        Dog.find({name: regex}).skip((perPage * pageNumber) - perPage).limit(perPage).exec(function (err, allDogs) {
            if (err) {
        ...
        Dog.find({}).skip((perPage * pageNumber) - perPage).limit(perPage).exec(function (err, allDogs) {
            if (err) {
              req.flash('error', err.message)
            }
